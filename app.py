...
all_macros = sorted(region_df['Macro_Region'].dropna().unique().tolist())
...
if selected_macros:
    filtered_states = sorted(region_df[region_df['Macro_Region'].isin(selected_macros)]['State/Province'].dropna().unique().tolist())
else:
...
if selected_macros:
    display_filter = display_filter[display_filter['Macro_Region'].isin(selected_macros)]
if selected_states:
...
if selected_macros:
    filtered_df = filtered_df[filtered_df['Macro_Region'].isin(selected_macros)]
...
sort_columns = ["Macro_Region", "State/Province", "Region Display Name", "County Name"]
